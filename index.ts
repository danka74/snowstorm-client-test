...
const getPage = (index: number) => {
  return ajax({
    ...
    method: "GET",
    url:
      "http://localhost:8080/MAIN%2FSNOMEDCT-SE/concepts/195967001/descendants?stated=false&offset=" +
        index || 0 + "&limit=50"
  }).pipe(
    //        tap(console.log),
    map(r => r.response)
  ...
  .pipe(
    ...
    mergeMap((concept) => {
      return ajax({
        ...
        method: 'GET',
        url:
          'http://localhost:8080/browser/MAIN%2FSNOMEDCT-SE/concepts/' +
          concept.conceptId,
      }).pipe(
    ...
    map((concept) =>
      concept.relationships.filter((rel: any) => rel.active === true),
    )
    // mapTo(1),
